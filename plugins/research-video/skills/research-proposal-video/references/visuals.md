...
数据图画面上的每条曲线、每根柱子都是一个事实主张，和口播里的数字同样要有出处。先找真实数据：公认的参考标准（例如生长曲线、诊断阈值表），团队已发表论文的表格和图（准确率、病例的特征贡献），公开的统计数据。原文只有图时，可以按像素读数：先用两个已知刻度确定比例，从零线量柱长或点位，在事实表里注明“从图中读取、近似值”。
